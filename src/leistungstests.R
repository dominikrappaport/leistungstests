...
leistungstests2023 <- read.csv("data/processed/Leistungsmessung202311.csv") %>%
  mutate(datum = as.Date("2023/11/7")) %>%
  mutate(V.O2.kg.rel = V.O2.kg/max(V.O2.kg))

leistungstests2024 <- read.csv("data/processed/Leistungsmessung202410.csv") %>%
  mutate(EUFett = as.integer(NA), EUKH = as.integer(NA), BWG = as.integer(NA), datum = as.Date("2024/10/1")) %>%
  mutate(V.O2.kg.rel = V.O2.kg/max(V.O2.kg))
...
leistungstests.plot <- leistungstests %>%
  ...
  scale_x_continuous(limits = c(10, 50), breaks = seq(10, 50, 5), labels = c("10", "15", "20", "25", "30", "35", "40", "45", expression(50~ml~O[2]/min/kg))) +
  scale_colour_manual(values = c("#FAAB18", "#1380A1"), labels = c("2023", "2024")) +
  bbc_style() +
...
leistungstests.plot <- leistungstests %>%
  filter(P > 50) %>%
  mutate(datum = as.factor(datum)) %>%
  ggplot(aes(x = V.O2.kg.rel, y = P, colour = datum)) +
  geom_point(size = 1) +
  stat_smooth(method = "lm",
              formula = y ~ x,
              geom = "smooth") +
