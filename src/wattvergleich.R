...
wattvergleich.favero <-wattvergleich.favero %>%
  ...
  filter(Timestamp >= 1096705521, Type == "Data", Message == "record", WattFavero > 0) %>%
  mutate(Timestamp = as_datetime(Timestamp + 631065600, tz = "Europe/Vienna")) %>%
...
wattvergleich <- wattvergleich.cyclus2 %>% 
  ...
  mutate(WattFavero = WattCyclus2 + WattAbweichung, WattAbweichungRelativ = abs(WattAbweichung)/WattCyclus2) %>%
  select(Lap, WattCyclus2, WattFavero, WattAbweichung, WattAbweichungRelativ)

# Plot results ------------------------------------------------------------

wattvergleich.diagram <- wattvergleich %>%
  select(Lap, WattCyclus2, WattFavero) %>%
  pivot_longer(cols = c(WattCyclus2, WattFavero), names_to = "Source", values_to = "Watt") %>%
