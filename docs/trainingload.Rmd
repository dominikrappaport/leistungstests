...
Übersicht über Trainingsstunden, TSS und CTL für die Jahre 2020 bis 2025. Als Zeitraum zum Berechnen des Durchschnitts wurden alle (bereits verstrichenen) Wochen des Jahres herangezogen und nicht nur die, an denen ein Training grundsätzlich möglich war. D. h., Urlaube, Krankenstände, Dienstreisen usw. drücken den Schnitt.
...
yearly_statistics <- tl %>% 
  ...
  rowwise() %>%
  mutate(
    training_hours_per_year = round(training_hours_per_year),
    training_hours_per_week = round(training_hours_per_week / days_in_year_elapsed(year) * 7, 2),
    TSS_per_year = round(TSS_per_year),
    TSS_per_week = round(TSS_per_week / days_in_year_elapsed(year) * 7),
    average_CTL = round(average_CTL)
  ) %>%
  rename(Jahr = year, 
         `Trainingsstunden` = training_hours_per_year,
         `Trainingsstunden pro Woche (Ø)` = training_hours_per_week,
         `TSS` = TSS_per_year,
         `TSS pro Woche (Ø)` = TSS_per_week,
         `CTL (Ø)` = average_CTL)
